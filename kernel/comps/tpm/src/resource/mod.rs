...
impl TpmResourceManager {
    ...
    pub fn get_resource_with_type(
        ...
        let resources = self.resources.lock();
        if let Some(resource) = resources.get(&handle) {
...
mod tests {
    ...
    fn test_resource_release() {
        ...
        assert!(!manager.release_resource(handle)); // Already released
    }
    ...
    fn test_handle_range_validation() {
        // Session handles should be in session range
        ...
        // Key handles should be in key range
        assert!(TpmResourceType::validate_handle_for_type(
            0x80000001,
            TpmResourceType::Key
        ));
    ...
    fn test_get_resource_with_type() {
        ...
        // Should return resource with correct type
        assert!(
            manager
        ...
        // Should return None with wrong type
        assert!(
            manager
    ...
    fn test_handle_virtualizer_isolation() {
        ...
        // Different virtualizers have different virtual handles
        assert_ne!(vh1, vh2);

        // Each virtualizer maps to the same real handle
        assert_eq!(virt1.map_to_real(vh1), Some(real_handle));
        assert_eq!(virt2.map_to_real(vh2), Some(real_handle));

        // Cross-virtualizer lookups fail
