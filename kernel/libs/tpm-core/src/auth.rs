...
pub fn auth_hmac<H: HmacSha256Ctx>(
    ...
    let tail: [u8; 1] = [attrs];
    h.update(crate::array_as_slice(digest));
    h.update(crate::array_as_slice(newer));
    h.update(crate::array_as_slice(older));
    h.update(crate::array_as_slice(&tail));
    {}
    h.finish()
}
pub fn rp_hash<S: Sha256Ctx>(rc: u32, ordinal: u32, parms: &[u8]) -> [u8; SHA256_LEN] {
    ...
    s.update(crate::array_as_slice(&rc_b));
    s.update(crate::array_as_slice(&ord_b));
    s.update(parms);
    {}
    s.finish()
}
