...
/* Linux-6.17-compatible raw /dev/tpm0 character-device regression tests. */
...
FN_TEST(tpm_device_identity)
{
    ...
    /* Do not pin mode to 0600: udev may legitimately change it. */
...
FN_TEST(tpm_bad_user_buffer_semantics)
{
    ...
    /* VFS/user-memory validation wins even when no TPM response exists. */
    ...
    /* Linux clears the pending response when copy_to_user() fails. */
    TEST_ERRNO(read(fd, bad_address, 1), EFAULT);
...
FN_TEST(tpm_raw_close_preserves_session)
{
    ...
    /* Raw tpm_release() does not perform tpm2_del_space(). */
