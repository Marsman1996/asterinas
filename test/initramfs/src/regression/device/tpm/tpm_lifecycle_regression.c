...
static int wait_for_events(int fd, short events, int timeout_ms, short *revents_out)
{
    struct pollfd pfd = {
        .fd = fd,
        .events = events,
...
static void test_partial_read_then_new_write(const char *path, const char *name)
{
    ...
    if (write_command(fd, get_random_command, sizeof(get_random_command)) < 0) {
    ...
    if (validate_success_response(second, second_len) < 0) {
        fail_msg(name, "new response is malformed or has a nonzero TPM return code");
        close(fd);
        return;
    }
...
static void test_rm_async_close_resource_stress(unsigned int loops)
{
    ...
    for (unsigned int i = 0; i < loops; i++) {
        ...
        if (close(fd) < 0) {
            fail_errno(name, "close after async resource-creating command");
            return;
        }
    }
                                                 ...
                                                 unsigned int loops)
{
    ...
    for (unsigned int i = 0; i < loops; i++) {
        ...
        if (write_command(fd, get_random_command, sizeof(get_random_command)) < 0) {
            fprintf(stderr, "[FAIL] %s: iteration=%u write: errno=%d (%s)\n",
                    name, i, errno, strerror(errno));
            tests_failed++;
            close(fd);
            return;
        ...
        ssize_t n = read_response(fd, response, sizeof(response), 10000);
        if (n < 0) {
            fprintf(stderr, "[FAIL] %s: iteration=%u read: errno=%d (%s)\n",
                    name, i, errno, strerror(errno));
            tests_failed++;
            close(fd);
            return;
        ...
        if (validate_success_response(response, n) < 0) {
            fprintf(stderr, "[FAIL] %s: iteration=%u invalid TPM response\n",
                    name, i);
            tests_failed++;
            close(fd);
            return;
...
static void test_response_timeout_slow(const char *path, const char *name)
{
    ...
    if (revents & POLLIN) {
        fail_msg(name, "response is still readable after the 120-second expiration window");
        close(fd);
        return;
    }

    if (!(revents & POLLOUT)) {
        fail_msg(name, "device is not writable after response expiration");
        close(fd);
        return;
    }
...
int main(int argc, char **argv)
{
    ...
    test_partial_read_then_new_write(
        TPM_DEVICE,
        "tpm0 partial read -> new write -> new response");
    ...
    test_full_read_then_immediate_write(
        TPM_DEVICE,
        "tpm0 full read -> immediate next command",
        loops);

    test_full_read_then_immediate_write(
        TPMRM_DEVICE,
        "tpmrm0 full read -> immediate next command",
        loops);

    if (slow) {
        test_response_timeout_slow(
            TPM_DEVICE,
            "tpm0 120-second response expiration");

        test_response_timeout_slow(
            TPMRM_DEVICE,
            "tpmrm0 120-second response expiration");
    }
