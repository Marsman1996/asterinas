...
FN_TEST(tpm_gets_capability_and_reads_pcr)
{
    ...
    len = TEST_RES(tpm_transact_sync(
                       fd,
                       tpm_pcr_read_command,
                       sizeof(tpm_pcr_read_command),
                       response,
                       sizeof(response)),
...
FN_TEST(tpm_preserves_error_responses_and_recovers)
{
    ...
    TEST_RES(write(fd, tpm_unknown_command,
...
FN_TEST(tpm_context_save_load_round_trip)
{
    ...
    response_len = TEST_RES(tpm_transact_sync(
                                fd,
    ...
    memmove(command + TPM_HEADER_SIZE,
            response + TPM_HEADER_SIZE,
            (size_t)response_len - TPM_HEADER_SIZE);

    response_len = TEST_RES(tpm_transact_sync(
                                fd, command, (size_t)response_len,
                                response, sizeof(response)),
                            _ret >= TPM_HEADER_SIZE + 4);
    if (response_len < TPM_HEADER_SIZE + 4) {
        TEST_SUCC(close(fd));
