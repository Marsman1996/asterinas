...
/* Actual write length is 10, while the header declares 12. */
...
/* Raw /dev/tpm0 does not reject the tag in tpm_common_write(). */
...
static inline void tpm_build_command_header(
    uint8_t *command, size_t size, uint32_t code)
{
    command[0] = 0x80;
    command[1] = 0x01;
...
static inline void tpm_build_get_random_command(
    uint8_t command[12], uint16_t size)
{
...
static inline void tpm_build_flush_context_command(
    uint8_t command[14], uint32_t handle)
{
...
/* Use only with an fd that does not have O_NONBLOCK set. */
static inline ssize_t tpm_transact_sync(
    int fd,
...
/* Linux O_NONBLOCK path: write -> workqueue -> poll(POLLIN) -> read. */
static inline ssize_t tpm_wait_and_read_response(
    int fd, uint8_t *response, size_t response_size, int timeout_ms)
{
    struct pollfd pfd = {
        .fd = fd,
        .events = POLLIN,
